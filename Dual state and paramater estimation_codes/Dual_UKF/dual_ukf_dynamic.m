...
xpbar = [1500; 12e4; 7e4]; % initial belief of parameter

Ps = diag([1, 1, 0.2]); %initial belief uncertainity
Pp = diag([200 2e4 1.5e4]);
Rs = 0.1*diag([1.2^2 1.2^2 0.45^2]); %process noise
Rp = diag([20^2 1.75e2^2 1.5e2^2]);

Q = diag([0.02^2 0.02^2 0.025^2]); %measurement noise
Hs = eye(3);
gamma = 0.998;
n = size(u1,1);
Xsbar(:,1) = xsbar;
